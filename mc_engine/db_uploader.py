...
def upload_best_frame(session_scored_dir: str, session_name: str) -> dict | None:
    ...
    if not os.path.exists(img_path):
        raw_dir = session_scored_dir.replace("_scored", "_raw").replace("/score/", "/raw/").replace("\\score\\", "\\raw\\")
    ...
    # Read internal token
    headers = {}
    data_dir = os.environ.get("MAGIC_CLICK_DATA")
    if not data_dir:
        # project-root/data
    ...
    try:
        ...
        log.warning(f"[DB UPLOAD] Could not read internal token from {secret_path}: {e}")
